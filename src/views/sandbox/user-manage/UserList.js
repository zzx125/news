...
export default function UserList() {
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:5000/api/users?_expand=role").then(res => {
            const list = res.data
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/api/regions").then(res => {
            const list = res.data
            setregionList(list)
        })
    }, [])
    ...
    const handleChange = (item)=>{
        ...
        axios.patch(`http://localhost:5000/api/users/${item.id}`,{
    ...
    const deleteMethod = (item) => {
        ...
        axios.delete(`http://localhost:5000/api/users/${item.id}`)
    }
    ...
    const updateFormOK = ()=>{
        updateForm.current.validateFields().then(value => {
            ...
            axios.patch(`http://localhost:5000/api/users/${current.id}`,value)
        })
    }
